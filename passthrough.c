 ...
 */
				...
				struct fuse_file_info *fi)
 {
	 ...
	 struct stat staux;
	 int remote_res = rstat(p_ctx->client_fd, p_ctx->client_fd2, path, &staux, p_ctx->rrobin);
	 if(remote_res==0){
		 stbuf->st_size = staux.st_size;
	 }

	 p_ctx->rrobin = (p_ctx->rrobin + 1) % NSERVERS;
 ...
 static int xmp_mkdir(const char *path, mode_t mode)
 {
	...
	if (res == -1)
		return -errno;
	Context *p_ctx = fuse_get_context()->private_data;
	pthread_mutex_lock(&p_ctx->mutex);

	res = rpmkdir(p_ctx->client_fd, p_ctx->client_fd2, path, mode);
	printf("res do mkdir no pass: %d\n", res);
	pthread_mutex_unlock(&p_ctx->mutex);
 ...
 static int xmp_rmdir(const char *path)
 {
	...
	res = rprmdir(p_ctx->client_fd, p_ctx->client_fd2, path);
	printf("res do rmdir no pass: %d\n", res);
	pthread_mutex_unlock(&p_ctx->mutex);
	 return 0;
 }
			 ...
			 struct fuse_file_info *fi)
 {
	 ...
	 pthread_mutex_lock(&p_ctx->mutex);
	 if (CACHE_ACTIVE) {
		char *cache_key = make_cache_key(path, offset);
		//check cache
		Indexmeta *file = malloc(sizeof(Indexmeta));
		int get_res = index_get(p_ctx->cache, cache_key, file);
		if (get_res == 0){
			size_t safe_size = (file->size > size) ? size : file->size;
			printf("file->data: %s\n", file->data);
			memcpy(buf, file->data, safe_size);
			res = safe_size;
		}
		else {
			//read part - contact server
			res = rpread(p_ctx->client_fd, p_ctx->client_fd2, path, buf, size, offset, p_ctx->rrobin);
			
			if (res != -1){
				Indexmeta value;
				value.data = buf;
				value.size = res;
				int cache_res = index_add(p_ctx->cache, cache_key, value);

				if (cache_res == -2) {
					printf("Failed to add to cache because Cache is full!\n");
				}
			}
			p_ctx->rrobin = (p_ctx->rrobin + 1) % NSERVERS;
		}
			  ...
			  off_t offset, struct fuse_file_info *fi)
 {
	 ...
	 if (CACHE_ACTIVE) {
		Indexmeta *file = malloc(sizeof(Indexmeta));
		int get_res = index_get(p_ctx->cache, path, file);
		if (get_res == 0){
			index_remove(p_ctx->cache, path);
		}
		free(file);
	 }
